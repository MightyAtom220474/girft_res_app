...
LD_CSV_PATH = os.path.join(BASE_DIR, "legacy_activity_weekly_normalised.csv")
...
for table in ["programme_categories", "staff_list", "programme_activity", "leave_calendar", "on_site_calendar"]:
...
if os.path.exists(SL_CSV_PATH):
    ...
    # Convert True/False or 'True'/'False' to integers 1/0
    df["must_change_password"] = df["must_change_password"].map(
        {True: 1, False: 0, "True": 1, "False": 0}
    ).fillna(1).astype(int)  # default to 1 if missing
...
if os.path.exists(LC_CSV_PATH):
    ...
    for _, row in df.iterrows():
        ...
        INSERT OR REPLACE INTO leave_calendar (
            staff_member, week_commencing, week_number, days_leave, created_at, updated_at
        ) VALUES (?, ?, ?, ?, ?, ?)
        """, (
            row.get("staff_member"),
            row.get("week_commencing"),
            row.get("week_number"),
            row.get("days_leave", 0),
            None,
            None
        ))
    conn.commit()
...
# Load legacy programme activity
if os.path.exists(LD_CSV_PATH):
    ...
    legacy_df["week_commencing"] = pd.to_datetime(
        legacy_df["week_commencing"], dayfirst=True, errors="coerce"
    ).dt.date
    for _, row in legacy_df.iterrows():
        cursor.execute("""
        INSERT OR REPLACE INTO programme_activity (
            staff_member, week_commencing, week_number,
            programme_category, activity_value, created_at, updated_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (
            row.get("staff_member"),
            row.get("week_commencing"),
...
conn.close()
print("Database setup complete – tables are ready for CSV load and runtime use")
